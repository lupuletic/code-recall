...
2. Update `src/claude_code_recall/__init__.py`.
...
5. Verify the local command with `uv run claude-code-recall --version`.
6. If the user is using the global tool install, reinstall with:
   `uv tool install --force /Users/lupuletic/TheHutGroup/claude-code-recall --with textual --with fastembed --with sqlite-vec`
7. Verify the installed command with `claude-code-recall --version`.
...
Only push a `vX.Y.Z` tag when `pyproject.toml`, `src/claude_code_recall/__init__.py`, and the tag version all match. The GitHub release workflow publishes the `claude-code-recall` distribution to PyPI through Trusted Publishing using the `pypi` environment.
