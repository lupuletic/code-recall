...
INIT = ROOT / "src" / "claude_recall" / "__init__.py"


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--tag", help="Optional release tag, e.g. v0.1.3")
    args = parser.parse_args(argv)

    versions = {
        "pyproject.toml": _read_pyproject_version(PYPROJECT),
        "src/claude_recall/__init__.py": _read_init_version(INIT),
    }
    if args.tag:
